...
const ParentDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (distance < 0.5) {
      setShowToast({ show: true, msg: "🚍 Van is nearly at the drop-off point!" });
      ...
      <div className="px-5 mt-6 space-y-8">
        ...
        <section className={`${theme.card} rounded-[2.5rem] p-8 shadow-xl border ${theme.border} text-center`}>
          ...
          <div className="flex gap-2 justify-center">
            {guardianPin.split("").map((char, i) => (
              <div key={i} className={`w-10 h-14 ${theme.inner} rounded-xl flex items-center justify-center text-2xl font-black ${theme.textMain} border ${theme.border}`}>
                {char}
              </div>
            ))}
          </div>
        ...
        <section>
          <div className="flex justify-between items-center mb-4 px-2">
            ...
            <span className={`flex items-center gap-1.5 text-[10px] font-black px-3 py-1 rounded-full ${activeChild.is_on_bus ? "text-emerald-600 bg-emerald-500/10" : "text-amber-600 bg-amber-500/10"}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${activeChild.is_on_bus ? "bg-emerald-500 animate-ping" : "bg-amber-500"}`} />
...
const HistorySection = ({ title, theme, items, isDropoff }: any) => (
  <section>
    <h3 className={`font-black ${theme.textMain} text-sm mb-4 px-2`}>{title}</h3>
    <div className={`${theme.card} rounded-3xl p-2 space-y-1 border ${theme.border}`}>
      {items && items.length > 0 ? (
        items.map((log: LogEntry) => (
          <HistoryItem
            key={log.id}
            icon={isDropoff ? <MapPin size={14} className="text-blue-600" /> : <CheckCircle2 size={14} className="text-emerald-500" />}
...
const HistoryItem = ({ icon, title, time, bus, theme, statusColor }: any) => (
  <div className={`flex items-center justify-between p-4 ${theme.inner} rounded-2xl mb-1`}>
    <div className="flex items-center gap-4">
      <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-white shadow-sm">{icon}</div>
      <div>
        <h4 className={`text-sm font-bold ${theme.textMain}`}>{title}</h4>
        <p className={`text-[10px] ${theme.textSub} font-medium`}>{time} • {bus}</p>
      </div>
    </div>
    <span className={`text-[10px] font-black px-2.5 py-1 rounded-full ${statusColor}`}>Verified</span>
  </div>
);
